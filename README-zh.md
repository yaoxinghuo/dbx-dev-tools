...
日常开发工具集合的 [DBX](https://github.com/t8y2/dbx) 插件——**沙箱离线运行，零联网、仅 1 项本地权限（`host.storage`，本地 UI 状态存储）——更轻更安全**。40+ 款开发工具仅 ~100KB：密码学生成、哈希/HMAC、AES、JWT/TOTP、RSA/证书、多进制编解码、时间戳与 Cron、IP 子网、文本 Diff、颜色对比度、Unicode 检查、乱码修复、正则测试等一应俱全。纯前端实现、universal 包、无原生 Sidecar。
