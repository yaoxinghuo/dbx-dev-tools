...
日常开发工具集合的 [DBX](https://github.com/t8y2/dbx) 插件——**沙箱离线运行，零联网、仅 1 项本地权限（`host.storage`，本地 UI 状态存储）**。42 款开发工具仅 ~150KB：集安全加解密、编解码与转义、文本与正则排版、时间与 Cron 计算、网络与系统运维、前端图像与标识生成等全场景工具于一体，满足日常开发高频需求。纯前端实现、universal 包、无原生 Sidecar。
...
<img width="2090" height="1412" alt="image" src="https://github.com/user-attachments/assets/7b43078d-d781-4e52-8449-9094872aba1e" />
...
**插件中心 → 插件商店**，搜索 **Dev 工具集** 点击安装。商店包经官方签名，无需额外设置。
...
**插件中心 → 已安装 → Dev 工具集**，只打开一个 `Dev 工具集` 工作台标签页；插件内左侧导航切换所有工具。标签页跨重启恢复，打开一次后保留即可。
